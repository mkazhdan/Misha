...
// Grid //
//////////
template< typename Data > Grid< Data >::Grid( int rX , int rY ) : resX(0) , resY(0) , values(NULL) { resize( rX , rY ); }

template< typename Data > Grid< Data >::~Grid( void ){ if( values ) resize(0,0); }

template< typename Data >
int Grid< Data >::resize( int rX , int rY )
{
...
template< typename Data >
Data& Grid< Data >::operator() (const unsigned int& i,const unsigned int& j){
...
template< typename Data >
const Data &Grid< Data >::operator() (const unsigned int& i,const unsigned int& j) const {
	unsigned int x=i,y=j;
	x%=resX;
	y%=resY;
	return values[x*resY+y];
...
Data Grid< Data >::operator() (const double& x,const double& y){
	...
	dx=x-x1;
	dy=y-y1;
	return (Data)( (*this)(x1,y1)*(1.0-dx)*(1.0-dy)+(*this)(x1+1,y1)*dx*(1.0-dy)+(*this)(x1,y1+1)*(1.0-dx)*dy+(*this)(x1+1,y1+1)*dx*dy );
}
template< typename Data >
Data Grid< Data >::operator() (const double& x,const double& y) const {
	...
	dx=x-x1;
	dy=y-y1;
	return (Data)( (*this)(x1,y1)*(Data(1.0)-dx)*(Data(1.0)-dy)+(*this)(x1+1,y1)*dx*(Data(1.0)-dy)+(*this)(x1,y1+1)*(Data(1.0)-dx)*dy+(*this)(x1+1,y1+1)*dx*dy );
}
template< typename Data >
template<class Real>
